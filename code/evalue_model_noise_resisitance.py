...
def main():
    ...
    # Load model
    model_path = 'model_used/best_model_CNN_Designedv2_Pristine.pth'
    ...
    for filename in os.listdir(folder_selected):
        if filename.endswith('.txt'):
            ...
            if input_data.size > 0:
                ...
                input_data = add_gaussian_noise(input_data, variance=0.50)

                input_data = torch.tensor(input_data, dtype=torch.float32).view(-1, 1, sequence_length)  # 增加批次维度
            ...
            # output_path = os.path.join(folder_selected, "output", output_filename)
            # np.savetxt(output_path, np.array(outputs), fmt="%.3f")

    result_filename = "noise_result_0.50.txt"
    result_path = os.path.join(folder_selected, "noise_output", result_filename)
